...
ogImage: ../../assets/og/variational-autoencoder.png
ogImageAlt: "A dense grid of molecular structure diagrams generated by a variational
  autoencoder, each a few rings and bonds drawn in black, blue and red."
---
...
<figure>
    <img class="plate" src="/images/encoder-decoder.png" alt="A diagram of an autoencoder: input data flows through an encoder into a small latent space z, then through a decoder back out to a reconstruction.">
...
lower-dimensional space is stochastic: the encoder outputs parameters to $q_\theta (z \mid x)$, which is a Gaussian probability density. We can sample
...
terms that depend on a single datapoint $l_i$. The total loss is then $\sum_{i=1}^N l_i$ for $N$ total datapoints. The loss function $l_i$
...
<figure>
    <img class="plate" src="/images/graphical-model-variational-autoencoder.png" width="50%" height="40px" style="max-width: 40%" alt="A graphical model: an unshaded circle for the latent variable z with an arrow to a shaded circle for the observed data x, enclosed in a plate repeated N times.">
...
<center>
<iframe title="Animated GIF" src="//giphy.com/embed/lqq0em9cuivVNWFwSX?html5=true" width="480" height="413" frameBorder="0" class="giphy-embed" allowFullScreen></iframe>
</center>
...
<center>
<iframe title="Animated GIF" src="//giphy.com/embed/h8rGTcMpkPVpCzru7q?html5=true" width="480" height="480" frameBorder="0" class="giphy-embed" allowFullScreen></iframe>
</center>
...
## <a id="mean-field"></a>Mean-field versus amortized inference 
...
<figure>
    <img class="plate" src="/images/reparametrization.png" alt="Two side-by-side computation graphs. On the left the random node z sits on the path from the parameters to the loss; on the right the randomness is moved into a separate node epsilon so z becomes a deterministic diamond.">
...
Is anything in this article confusing or can any explanation be improved? Please submit a [pull request](https://github.com/altosaar/jaan.io/blob/master/_posts/blog/2016-07-18-what-is-variational-autoencoder-vae-tutorial.md), [tweet me](https://twitter.com/thejaan), or [email me](mailto:j@jaan.io) :)
