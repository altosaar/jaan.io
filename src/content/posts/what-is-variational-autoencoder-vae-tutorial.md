...
<figure>
    <img class="ink" src="/images/encoder-decoder.png" alt="A diagram of an autoencoder: input data flows through an encoder into a small latent space z, then through a decoder back out to a reconstruction.">
...
<figure>
    <img class="ink" src="/images/graphical-model-variational-autoencoder.png" width="50%" height="40px" style="max-width: 40%" alt="A graphical model: an unshaded circle for the latent variable z with an arrow to a shaded circle for the observed data x, enclosed in a plate repeated N times.">
...
<figure>
    <img class="ink" src="/images/reparametrization.png" alt="Two side-by-side computation graphs. On the left the random node z sits on the path from the parameters to the loss; on the right the randomness is moved into a separate node epsilon so z becomes a deterministic diamond.">
